    ...
    return Promise.all([
      ...
      if (!!limit && !!limit.last_black_to_normal_at) {
        ...
        if (lastBlackToNormalAtUTC8 > startOfDayAt) {
          console.log(`黑名單解除時間(當日) lastBlackToNormalAtUTC8: ${lastBlackToNormalAtUTC8} 晚於當天 ${startOfDayAt}`)
          startOfDayAt = lastBlackToNormalAtUTC8
        }
        if (lastBlackToNormalAtUTC8 > startOfMonthAt) {
          console.log(`黑名單解除時間(當月) lastBlackToNormalAtUTC8: ${lastBlackToNormalAtUTC8} 晚於當月 ${startOfMonthAt}`)
      ...
      return btmRiskcontrol.fetchDefaultLimit(limit.role)
        .then(defaultLimit => {
          ...
          console.log(`撈出用戶 ${tx.customerId} 交易紀錄 edd 條件一 ${limitlevel1}, 條件二 ${limitlevel2}`)
          ...
          if (!!limit && !!limit.last_black_to_normal_at) {
            ...
            lastBlackToNormalAtUTC8 = new Date(lastBlackToNormalAtUTC0.getTime() + 8 * 60 * 60 * 1000);
            if (lastBlackToNormalAtUTC8 > startOfDayAt) {
              console.log(`黑名單解除時間(edd 門檻一) lastBlackToNormalAtUTC8: ${lastBlackToNormalAtUTC8} 晚於當日 ${startOfDayAt}`)
              level1DaysStartAt = lastBlackToNormalAtUTC8
            }
            if (lastBlackToNormalAtUTC8 > startOfMonthAt) {
              console.log(`黑名單解除時間(edd 門檻二) lastBlackToNormalAtUTC8: ${lastBlackToNormalAtUTC8} 晚於當月 ${startOfMonthAt}`)
              level2DaysStartAt = lastBlackToNormalAtUTC8
            }
          }
          console.log(`撈出用戶 ${tx.customerId} 交易紀錄 edd 條件一天數 ${level1DaysStartAt}, 條件二天數 ${level2DaysStartAt}`)
          ...
          return Promise.all([
          ...
          ]).then(([txSumTodayStr, txSumThisMonthStr, txSumLevel1Str, txSumLevel2Str, res]) => {
            ...
            if (BN(txSumTodayStr).gte(BN(limit.daily_limit))) {
              console.log(`用戶 ${tx.customerId} ˋ撞到日限額`)
              isBlock = true
            ...
            if (BN(txSumThisMonthStr).gte(BN(limit.monthly_limit))) {
              console.log(`用戶 ${tx.customerId} ˋ撞到月限額`)
              isBlock = true
            ...
            if (level1Flag || level2Flag) {
              ...
              if (level1Flag) {
                ...
                console.log(`EDD 門檻 1 撞到限額，用戶: ${tx.customerId}, txSumLevel1Str: ${txSumLevel1Str}, limit.limitlevel1: ${limit.limitlevel1}, level1DaysStartAt: ${level1DaysStartAt}`)
              }
              if (level2Flag) {
                alertStr = `EDD 門檻 2 超過交易限額 ${limitlevel2}，累積交易金額天數 ${level2Days}`
                console.log(`EDD 門檻 2 撞到限額，用戶: ${tx.customerId}, txSumLevel2Str: ${txSumLevel2Str}, limit.limitlevel2: ${limit.limitlevel2}, level2DaysStartAt: ${level2DaysStartAt}`)
              }
            ...
            console.log('isEDDBlock', isEDDBlock)
            if (isEDDBlock) {
              console.log('觸發 EDD block');
              // 郵件選項
              const mailOptions = {
