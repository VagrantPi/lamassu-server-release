    ...
    return Promise.all([
      ...
      return btmRiskcontrol.fetchDefaultLimit(limit.role)
        .then(defaultLimit => {
          ...
          level1DaysStartAtUTC0 = btmRiskcontrol.dayMuteNUTC0(level1Days)
          level2DaysStartAtUTC0 = btmRiskcontrol.dayMuteNUTC0(level2Days)
          logger.info(`${tx.customerId} 撈取用戶交易紀錄，原始 EDD 1 level1Days: ${level1Days}, level1DaysStartAtUTC0: ${level1DaysStartAtUTC0}`)
          logger.info(`${tx.customerId} 撈取用戶交易紀錄，原始 EDD 2 level2Days: ${level2Days}, level2DaysStartAtUTC0: ${level2DaysStartAtUTC0}`)
          ...
          if (!!limit && !!limit.last_black_to_normal_at) {
            ...
            logger.info(`${tx.customerId} 撈取用戶交易紀錄，lastBlackToNormalAtUTC0: ${lastBlackToNormalAtUTC0}`)
            ...
            if (lastBlackToNormalAtUTC0 > startOfDayAtUTC0) {
              ...
              level1DaysStartAtUTC0 = lastBlackToNormalAtUTC0
            }
            if (lastBlackToNormalAtUTC0 > startOfMonthAtUTC0) {
              ...
              level2DaysStartAtUTC0 = lastBlackToNormalAtUTC0
            }
          }
          logger.info(`${tx.customerId} 撈取用戶交易紀錄 edd 條件一天數 ${level1DaysStartAtUTC0}, 條件二天數 ${level2DaysStartAtUTC0}`)
          ...
          return Promise.all([
            ...
            btmTx.customerHistorySum(tx.customerId, level1DaysStartAtUTC0),
            btmTx.customerHistorySum(tx.customerId, level2DaysStartAtUTC0),
            ...
            if (level1Flag || level2Flag) {
              ...
              if (level1Flag) {
                ...
                logger.info(`${tx.customerId} EDD 門檻 1 撞到限額, txSumLevel1Str: ${txSumLevel1Str}, limit.level1: ${limit.level1}, level1DaysStartAtUTC0: ${level1DaysStartAtUTC0}`)
              }
              if (level2Flag) {
                alertEddStr = `EDD 門檻 2 超過交易限額 ${limitlevel2}，累積交易金額天數 ${level2Days}`
                logger.info(`${tx.customerId} EDD 門檻 2 撞到限額, txSumLevel2Str: ${txSumLevel2Str}, limit.level2: ${limit.level2}, level2DaysStartAtUTC0: ${level2DaysStartAtUTC0}`)
              }
