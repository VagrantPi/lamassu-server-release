...
function addOrUpdateCustomer (customerData, deviceId, config, isEmailAuth) {
  ...
  return getFunc(customerKey)
    ...
    .then(([customer, sumsubInfo, limit]) => {
      console.log('撈出交易紀錄的當月 1 號時間 startAt：', startAt)
      if (sumsubInfo && sumsubInfo.ban_expire_date) {
        ...
        console.log('解除 告誡名單 帳號時間：', cibExpireDate)
        if (cibExpireDate > startAt) {
          console.log('解除 告誡名單 帳號時間比當月 1 號時間晚，更新 startAt')
          startAt = cibExpireDate;
          console.log('更新後的 startAt：', startAt)
        }
      }
      if (!!limit && !!limit.last_black_to_normal_at) {
        ...
        console.log('解除 Ban 帳號時間：', lastBlackToNormalAtUTC8)

        if (lastBlackToNormalAtUTC8 > startAt) {
          console.log('解除 Ban 帳號時間比 startAt 晚，更新 startAt')
      ...
      console.log(`撈取用戶 ${customer.id} 交易紀錄，startAt: ${startAt}(UTC+8), ${startAtUtc0}(UTC+0) `)
    ...
    .then(([customer, limit]) => {
      ...
      console.log('default_daily_limit', default_daily_limit.toString())
      console.log('default_monthly_limit', default_monthly_limit.toString())
