...
const whitelist = require('../btm_whitelist')
const custimInfoRequest = require('../btm_custom_info_reuqest')
const cibs = require('../btm_cibs')
const db = require('../db')
  ...
  return cashInAtomic.atomic(machineTx, pi)
    .then(r => {
      ...
      return Promise.all([
        ...
        customer.getCustomerById(machineTx.customerId)
      ])
        .then(([
          customInfoRequests,
          customerData
          ...
          // 檢查客戶資訊 - 生份證 選項是否開啟
          const customInfoRequest = _.find(x => ['身分證', 'id card'].includes(x.customRequest.name), customInfoRequests)
          if (customInfoRequest) {
            infoRequestId = customInfoRequest.id
          }
          if (infoRequestId == '') {
            idCardBlock = true
          }
          ...
          // 檢查客戶資訊 - 生份證 選項是否開啟
          const idData = _.filter(
            item => item.info_request_id === infoRequestId,
            customerData.customInfoRequestData
          )

          if (idData.length !== 1) {
            idCardBlock = true
          } else {
            idCard = idData[0].customer_data.data 
          }

          promises.push(checkForCIBS(idCard.trim().toUpperCase()))
          return Promise.all(promises)
            .then(([
              ...
              console.log("cibsItems", cibsItems);
              ...
              } else if (_.some(it => it.address === updatedTx.toAddress)(whitelistItems)) {
                // 檢查白名單
                console.log('是白名單');
                whitelisted = true
              ...
              if (cibsItems.length > 0) {
                console.log('為告誡名單 ', idCard);
                idCardBlock = true
              }
              ...
              .then(tx => _.set('blacklisted', blacklisted || !whitelisted || idCardBlock, tx)) // 不在白名單內，則走黑名單阻擋
              .then(tx => _.set('addressReuse', addressReuse, tx))
              .then(tx => _.set('validWalletScore', _.isNil(walletScore) ? true : walletScore.isValid, tx))
              .then(tx => {
                if (infoRequestId != '') {
                  console.log(`delete machineTx.customerId: ${machineTx.customerId}, infoRequestId: ${infoRequestId}`);
                  
                  deleteCustomInfoRequest(machineTx.customerId, infoRequestId)
                }
                return tx
              })
        })
...
function checkForCIBS (pid) {
  return cibs.blocked(pid)
}

function deleteCustomInfoRequest (customerId, infoRequestId) {
  return custimInfoRequest.deleteItem(customerId, infoRequestId)
}
