...
function updateCustomer (req, res, next) {
  ...
  // Step 1: 獲取自定義信息請求
  return customInfoRequests.getCustomInfoRequests(true)
    .then(customRequests => {
      // 篩選出包含 '身分證' 或 'id card' 的請求
      const filteredRequests = customRequests.filter(request => 
        request.name.includes('身分證') || request.name.includes('id card')
      );
      ...
      // Step 2: 獲取客戶數據
      return customers.getCustomerById(patch.customerId)
        .then(customerData => {
          // Step 3: 找出符合條件的對象
          const idData = customerData.customInfoRequestData.filter(data => 
            requestIds.includes(data.info_request_id)
          );
          ...
          if (idCardBlock) {
              // 使用 notifier 通知前端該用戶為黑名單
              notifier.notifyIfActive('compliance', 'blacklistNotify', { customerId: patch.customerId, message: 'This user is blacklisted.' })
                .then(() => {
                  // 可以選擇在這裡處理成功的通知
                  console.log('Blacklist notification sent for customer:', patch.customerId);
                })
                .catch(error => {
                  // 錯誤處理
              ...
              // 可能需要終止後續處理，根據具體需求
              return respond(req, res, { message: 'This user is blacklisted.' }, 403); // 返回403 Forbidden
          }
