...
function triggerSuspend (req, res, next) {
  ...
  logger.info(`${id}, days ${days}, suspensionDuration: ${JSON.stringify(suspensionDuration)}`)

  Promise.all([
    btmRiskcontrol.fetchTxVolume(id),
    customers.getById(id)
  ])
    .then(([limit, customer]) => {
      if (!customer) {
      ...
      logger.info(`${id} filteredItem.triggerType ${filteredItem.triggerType}`)
      logger.info(`${id} filteredItem.suspensionDays ${filteredItem.suspensionDays}`)
      logger.info(`${id} filteredItem.limit ${JSON.stringify(limit)}`)
      switch (filteredItem.triggerType) {
        case 'txVolume':
          limitStr = 0;
          if (filteredItem.suspensionDays === 1) {
            limitStr = `客製化日限額 ${limit.daily_limit}`;
          } else if (filteredItem.suspensionDays === 30) {
            limitStr = `客製化月限額 ${limit.monthly_limit}`;
          }

          console.log('filteredItem', filteredItem)
          alertStr = `${filteredItem.thresholdDays} 日內 ${limitStr}，封鎖 ${filteredItem.suspensionDays} 日`
          break;
        case 'txVelocity':
          alertStr = `${filteredItem.thresholdDays} 日內購買超過 ${limitStr} 次，封鎖 ${filteredItem.suspensionDays} 日`
          break;
        default:
          break;
      }
      ...
      logger.info(`${id} 郵件 alertStr:`, alertStr);
      // 發送郵件
      return emailSender.sendMail(mailOptions).then(result => {
        logger.info(`${id} 郵件發送成功:`, result);
      ...
      .catch(error => {
        ...
        logger.error(`${id} alertStr:`, alertStr);
        return customers.update(id, { suspendedUntil: add(suspensionDuration, new Date()) })
          .then(customer => {
            notifier.complianceNotify(settings, customer, req.deviceId, 'SUSPENDED', days)
            return respond(req, res, { customer })
...
function addOrUpdateCustomer (customerData, deviceId, config, isEmailAuth) {
  ...
  let startAtUtc0 = btmRiskcontrol.firstDayOfMonthUTC0()
  let startAt = new Date(startAtUtc0.getTime() + 8 * 60 * 60 * 1000);
  ...
  return getFunc(customerKey)
    ...
    .then(customer => {
      return btmApi.fetchIdNumber(customer.id)
        ...
        .catch(() => {
          ...
          return Promise.all([customer, null])
        })
    })
    ...
    .then(([customer, sumsubInfo, limit]) => {
      ...
      logger.info(`${customer.id} 撈取用戶交易紀錄，startAt: ${startAt}(UTC+8), ${startAtUtc0}(UTC+0) `)
    ...
    .then(([customer, limit]) => {
      ...
      logger.info(`${customer.id} default_daily_limit`, default_daily_limit.toString())
      logger.info(`${customer.id} default_monthly_limit`, default_monthly_limit.toString())
      logger.info(`${customer.id} limit.daily_limit`, limit.daily_limit.toString())
      logger.info(`${customer.id} limit.monthly_limit`, limit.monthly_limit.toString())
      logger.info(`${customer.id} limit.role`, limit.role)

      // 如果為黑名單，則直接返回，不往下檢查，
      if (limit.role === 3) {
        logger.info(`${customer.id} limit.role === 3, is block, reject cash-in flow`, limit.role)
      ...
      logger.info(`${customer.id} 日限額多賽入，當日塞入（預設日限額 - 客制日限額）`);
      logger.info(`${customer.id} =>${default_daily_limit.toString()} - ${limit.daily_limit} = ${dayLimit.toString()}`)
      ...
      logger.info(`${customer.id}  判斷月限額是否多塞紀錄，現在日期為 ${_now}(UTC+8)`);
      ...
      logger.info(`${customer.id} 月限額多塞入，前日塞入（預設月限額 - 客制月限額）`);
      logger.info(`${customer.id} =>${default_monthly_limit.toString()} - ${limit.monthly_limit} = ${monthLimit.toString()}`)
    ...
    .then(([customer, limit]) => {
      return btmMockTxHistoryLogs.insertLog(customer.id, deviceId, default_daily_limit, default_monthly_limit, limit_daily_limit, limit_monthly_limit, dayLimit, monthLimit, startAt, banExpireDateRaw, banExpireDate, limit.role)
        .then(() => customer)
    })
    .then(customer => {
