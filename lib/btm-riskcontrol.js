...
function fetchTxVolume (customerId) {
  const sql = `SELECT role, last_black_to_normal_at, daily_limit, monthly_limit, level1, level2, level1_days, level2_days FROM btm_risk_control_customer_limit_settings WHERE customer_id = $1`
...
function fetchDefaultLimit (role) {
  const sql = `SELECT daily_limit, monthly_limit, level1, level2, level1_days, level2_days FROM btm_risk_control_limit_settings WHERE "role" = $1;`
...
function markCustomerEddType(customer_id, edd_type) {
  const sql = `UPDATE "public"."btm_risk_control_customer_limit_settings" SET "edd_type" = $2, "updated_at" = NOW(), "role" = 3 WHERE "customer_id" = $1;`
  return db.none(sql, [customer_id, edd_type])
}
