...
function fetchTxVolume (customerId) {
  const sql = `SELECT daily_limit, monthly_limit FROM btm_risk_control_customer_limit_settings WHERE customer_id = $1`
...
function periodTodayUTC8() {
  ...
  const period = (todayMidnight.getTime() - now.getTime()) / 1000;
  const nowUTC8 = new Date(now.getTime() + period * 1000);
  const ISOString = nowUTC8.toISOString();
  const formattedString = ISOString.replace('Z', '+00').replace('T', ' ');
...
function periodThisMonthUTC8() {
  ...
  const period = (lastDayOfMonth.getTime() - now.getTime()) / 1000;
  const nowUTC8 = new Date(now.getTime() + period * 1000);
  const ISOString = nowUTC8.toISOString();
  const formattedString = ISOString.replace('Z', '+00').replace('T', ' ');
  return formattedString;
