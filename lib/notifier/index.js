        ...
        .then(defaultLimit => {
          ...
          if (!!limit && !!limit.last_black_to_normal_at) {
            ...
            if (lastBlackToNormalAtUTC0 > startOfDayAtUTC0) {
              logger.info(`${tx.customerId} 黑名單解除時間(edd 門檻一) lastBlackToNormalAtUTC0: ${lastBlackToNormalAtUTC0} 晚於當日 ${startOfDayAt}`)
              level1DaysStartAt = lastBlackToNormalAtUTC0
            }
            if (lastBlackToNormalAtUTC0 > startOfMonthAtUTC0) {
              logger.info(`${tx.customerId} 黑名單解除時間(edd 門檻二) lastBlackToNormalAtUTC0: ${lastBlackToNormalAtUTC0} 晚於當月 ${startOfMonthAt}`)
