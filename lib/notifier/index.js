...
function transactionNotify (tx, rec) {
  ...
  logger.info('!! before settingsLoader.loadLatest()')
  ...
  return settingsLoader.loadLatest().then(settings => {
    ...
    logger.info('readyToNotify', readyToNotify)
    logger.info('!highValueTx', !highValueTx)
    logger.info('readyToNotify && !highValueTx', readyToNotify && !highValueTx)
    ...
    logger.info('!!zeroConf', zeroConf)
    logger.info('!!notificationsEnabled', notificationsEnabled)
    logger.info('!!highValueTx', highValueTx)
    logger.info('!!isCashOut', isCashOut)
    logger.info('!!rec.isRedemption', rec.isRedemption)
    logger.info('!!rec.error', rec.error)
    ...
    return Promise.all([
      ...
      logger.info(`${tx.customerId} 撈取用戶交易紀錄，日限額 startOfDayAt: ${startOfDayAt}(UTC+8), ${startOfDayAtUTC0}(UTC+0), 月限額 startOfMonthAt: ${startOfMonthAt}(UTC+8), ${startOfMonthAtUTC0}(UTC+0)`)
        ...
        .then(defaultLimit => {
          ...
          if (!!limit && !!limit.last_black_to_normal_at) {
            ...
            if (lastBlackToNormalAtUTC8 > startOfDayAt) {
              logger.info(`${tx.customerId} 黑名單解除時間(edd 門檻一) lastBlackToNormalAtUTC8: ${lastBlackToNormalAtUTC8} 晚於當日 ${startOfDayAt}`)
              level1DaysStartAt = lastBlackToNormalAtUTC8
            }
            if (lastBlackToNormalAtUTC8 > startOfMonthAt) {
              logger.info(`${tx.customerId} 黑名單解除時間(edd 門檻二) lastBlackToNormalAtUTC8: ${lastBlackToNormalAtUTC8} 晚於當月 ${startOfMonthAt}`)
          ...
          return Promise.all([
            ...
            console.log('eppayClient res', res)
            ...
            if (level1Flag || level2Flag) {
              ...
              if (level1Flag) {
                ...
                logger.info(`${tx.customerId} EDD 門檻 1 撞到限額, txSumLevel1Str: ${txSumLevel1Str}, limit.limitlevel1: ${limit.limitlevel1}, level1DaysStartAt: ${level1DaysStartAt}`)
              }
              if (level2Flag) {
                alertStr = `EDD 門檻 2 超過交易限額 ${limitlevel2}，累積交易金額天數 ${level2Days}`
                logger.info(`${tx.customerId} EDD 門檻 2 撞到限額, txSumLevel2Str: ${txSumLevel2Str}, limit.limitlevel2: ${limit.limitlevel2}, level2DaysStartAt: ${level2DaysStartAt}`)
              }
